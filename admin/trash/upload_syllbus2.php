        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title"> Upload Syllabus </h3>
        <form enctype="multipart/form-data" action="" method="POST" >
        <div class="section-title"><h2>2/2 <a href="upload_syllabus1.php">Back</a></h2></div>
...
$insert = "insert into syllabus_copy(sfile_name,evaluate_ID) values('$newfilename','$evaluate_id')";
$update = "update syllabus_copy set sfile_name='$newfilename' where evaluate_ID='$evaluate_id'";
//echo '<pre>';

$select = "select * from syllabus_copy where evaluate_ID='$evaluate_id'";
...
if($exec->num_rows==0){
  if (move_uploaded_file($_FILES['filename']['tmp_name'],$uploaddir . $newfilename)) {
    ...
    header("refresh:0; url=upload_syllabus1.php");
    }
    else{
  ...
  if (move_uploaded_file($_FILES['filename']['tmp_name'],$uploaddir . $newfilename)) {
    ...
    $file_name = mysqli_fetch_assoc($exec);
    $oldfilename=$file_name['sfile_name'];
    ...
    if($result){
      echo "<script> alert('Syllabus uploaded successfully')</script>";
      echo "<script type='text/javascript'>window.location='upload_syllabus1.php';</script>";
    }
    else{
      echo "Error : ".mysqli_error();
    }
