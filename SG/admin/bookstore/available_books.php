              ...
              <div class="col-md-12">
                <div class="d-sm-flex justify-content-between align-items-center transaparent-tab-border {">
                  <ul class="nav nav-tabs tab-transparent" role="tablist">
                    <li class="nav-item">
                      <a class="nav-link active" href="#">Available Books</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link" href="requested_books.php?dept=All&sem=All" role="tab">Requested Books</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link" href="sold_books.php?dept=All&sem=All">Sold Books</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link" href="reported_books.php?dept=All&sem=All">Reported Books</a>
                    </li>
                  </ul>
                </div>
...
include("../includes/connect.php");
  ...
  if($dept == 'All' || $sem == 'All')
  {
    $query_BookStore = mysqli_query($conn,"SELECT * FROM bookstore WHERE BookStore_BuyStatus = '$status'");
  }
  else
  {
    $query_BookStore = mysqli_query($conn,"SELECT * FROM bookstore WHERE BookStore_BuyStatus = '$status' AND BookStore_Evaluate_ID in 
                        ...
                        <div class="card">
                          <div class="card-body text-center">
                            ...
                            <form class="form" method="post" action="view_details.php">
                              <button type="details" name="details" value=<?php echo $row_BookStore['BookStore_Book_ID'];?> style="margin-top: 10px; width: 100%" class="btn btn-outline-warning">View Detalis</button>
                            </form>
                            <form class="form" method="post" action="includes/process.php">
                              <button type="delete" name="delete" value=<?php echo $row_BookStore['BookStore_Book_ID'];?> style="margin-top: 10px; width: 100%" class="btn btn-outline-danger">Delete</button>
                            </form>
                          </div>
                        </div>
                      </div>
...
if(isset($_POST['filter'])){
  ...
  $sem = $_POST['sem'];
  echo "<script type='text/javascript'>window.location='available_books.php?dept=$dept&sem=$sem';</script>";
}
?>